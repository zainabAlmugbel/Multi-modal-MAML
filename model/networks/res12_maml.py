...
class ResNetMAML(nn.Module):
    ...
    def forward(self, x, params = None, embedding = False):
        ...
        #print("--------, in network")    
        #print(x.size, x.shape) #[4, 640, 2, 2] for support or [32, 640, 2, 2] for query
        
        if self.keep_avg_pool:
            x = self.avgpool(x) #[4,640,1,1]
    
        x = x.view(x.size(0), -1) #[4,640]
        
        if embedding:
            # flatten the emb for dim=3, x = x.view(batch_size, C, H * W) x.size(0), x.size(1), x.size(2)*x.size(3)
            # x = x.permute(0, 2, 1)  # shape: [batch_size, H*W, C] c: channel or embedding
            #now it is ready for attention
            ...
            #x = x.view(x.size(0), -1) #[4,640]
            # Apply Linear Layer on image
            logits = F.linear(x, weight=params['fc.weight'], bias=params['fc.bias'])
   
            return logits # changed was only logits
