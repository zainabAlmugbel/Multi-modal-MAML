...
from model.networks import SentenceEncoder
...
class FSLTrainer(Trainer):
    def __init__(self, args):
        ...
        #self.text_backbone = SentenceEncoder(800, use_cuda) #800 0r 128(32*4)
        self.fusion_fc = nn.Linear(2*32, 32) # image size
            ...
            if osp.exists(osp.join(*args.para_init.split('/')[:-1], 'PCAStats-{}.dat'.format(args.backbone_class))):
                ...
                for batch in tqdm(self.class_loader, desc='Get Embeddings', ncols=50):
                    if torch.cuda.is_available():
                        c_data, c_kg, c_label = batch[0].cuda(), batch[1].cuda(), batch[-1].cuda()
                    else:
                        c_data, c_kg, c_label = batch[0], batch[1].cuda(), batch[-1]
                    #print("batch: ",c_data.shape, c_kg.shape, c_label.shape)    
                    ...
                    with torch.no_grad():
                        ...
                        for j in range(int(np.ceil(c_data.shape[0] / 128))):
                            #print("c_data: ", c_data.shape,c_data.shape[0])
                            inst_emb.append(self.model.encoder(c_data[j*128:min((j+1)*128, c_data.shape[0]), :], embedding = True))
                            #kg_emb.append(self.model.encoder(c_kg[j*128:min((j+1)*128, c_kg.shape[0]), :], embedding = True))
                        inst_emb = torch.cat(inst_emb)
                        #kg_emb = torch.cat(kg_emb)
    ...
    def train(self):
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            for batch in self.train_loader:
                self.train_step += 1
                #kg here retrieved from the get
                data, kg, gt_label = batch
                #print(data)
                #print("-----------------")
                #print(kg)
                if torch.cuda.is_available():
                    data = data.cuda()
                    kg = kg.cuda().type(torch.cuda.FloatTensor)
                else:
                    kg = kg.float()
                ...
                sup_kg = kg[:args.way * args.shot]#sup_kg_elements 
             

                qry_kg = kg[args.way * args.shot:]
                ...
                #logits,imgfeat,txtfeat,fusedfeat = self.model(support, query,sup_kg,qry_kg)
                logits = self.model(support, query,sup_kg,qry_kg)
                #print("logiits: ",logits)
                #fuse the logits of image and kg then send to cross entropy
                loss = F.cross_entropy(logits, label)
                tl2.add(loss.item())
                    # Initialize visualizer
                #         # Initialize visualizer
                # visualizer = EmbeddingVisualizer()
                # text_embeddings= txtfeat.detach().numpy()
                # image_embeddings =  imgfeat.detach().numpy()
                # categories = label
                # # # Visualize linear fusion
                # print("Visualizing linear fusion...")
                # plt_fusion = visualizer.visualize_linear_fusion(text_embeddings, 
                # image_embeddings,
                # fusion_weights=[0.5, 0.5],  # More weight to text
                # labels=categories
                # )
                # plt_fusion.savefig("linear_fusion_visualization.png", dpi=300, bbox_inches='tight')
                # print("Visualization saved as 'linear_fusion_visualization.png'")
                
                # # Visualize embedding space
                # print("Visualizing embedding space...")
                # plt_space = visualizer.visualize_fusion_space(
                #     text_embeddings,
                #     image_embeddings,
                ...
                #     n_components=2
                # )
                # plt_space.savefig("embedding_space_visualization.png", dpi=300, bbox_inches='tight')
                # print("Visualization saved as 'embedding_space_visualization.png'")
                
                # # Visualize impact of different fusion weights
                # print("Visualizing weight impact...")
                # plt_weights = visualizer.visualize_fusion_weights_impact(
                #     text_embeddings,
                #     image_embeddings,
                #     labels=categories,
                #     weight_steps=6
                # )
                # plt_weights.savefig("weight_impact_visualization.png", dpi=300, bbox_inches='tight')
                # print("Visualization saved as 'weight_impact_visualization.png'")

                # # Visualize feature importance
                # print("Visualizing feature importance...")
                # plt_feat = visualizer.visualize_feature_importance(
                #     text_embeddings,
                #     image_embeddings,
                #     fusion_weights=[0.7, 0.3],
                #     n_top_features=8
                # )
                # plt_feat.savefig("feature_importance_visualization.png", dpi=300, bbox_inches='tight')
    ...
    def evaluate(self, data_loader):
        ...
        for i, batch in enumerate(data_loader, 1):
            if torch.cuda.is_available():
                data, kg = batch[0].cuda(), batch[1].cuda().type(torch.cuda.FloatTensor)
            else:
                data, kg = batch[0], batch[1].float()


            
            #add support kg and query kg
            sup_kg = kg[:args.way * args.shot]
            qry_kg = kg[args.way * args.shot:]
            support = data[:args.way * args.shot]
            query = data[args.way * args.shot:]
            logits = self.model.forward_eval(support, query, sup_kg, qry_kg)
            for e in self.running_dict:
            ...
            record[i-1, 1] = acc
            del data, kg, sup_kg, qry_kg, support, query, logits, loss
    ...
    def evaluate_test(self):
        ...
        for e in self.running_dict:
            self.running_dict[e]['mean_copy'] = deepcopy(self.running_dict[e]['mean'])
            self.running_dict[e]['var_copy'] = deepcopy(self.running_dict[e]['var'])        
        record = np.zeros((100, 2)) # loss and acc
        label = torch.arange(args.eval_way, dtype=torch.int16).repeat(args.eval_query)
        label = label.type(torch.LongTensor)
        if torch.cuda.is_available():
            label = label.cuda()
        ...
        #for ind in enumerate(self.test_loader, 1):
        #    print("ind: ", ind)
        #    pdb.set_trace()
        for i, batch in tqdm(enumerate(self.test_loader, 1)):
            #print("i: ", i, "batch",  len(batch), "batch[0]", batch[0].shape)
            if torch.cuda.is_available():
                data, kg = batch[0].cuda(), batch[1].cuda().type(torch.cuda.FloatTensor)
            else:
                data, kg = batch[0], batch[1].float()  
            ...
            record[i-1, 0] = loss.item()
            record[i-1, 1] = acc
            del data, support, query, kg, sup_kg, qry_kg, logits, loss
            torch.cuda.empty_cache()
    ...
    def evaluate_test_cross_shot(self):
        ...
        num_shots = [1, 5, 8 ]
        record = np.zeros((100, len(num_shots))) # loss and acc
        label = torch.arange(args.eval_way, dtype=torch.int16).repeat(args.eval_query)
        label = label.type(torch.LongTensor)
        if torch.cuda.is_available():
            label = label.cuda()
